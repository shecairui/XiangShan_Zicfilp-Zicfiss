...
class DecodeStage(implicit p: Parameters) extends XSModule
  ...
  /** at least 1 instruction decoded by simple decoders is illegal */
  val illegalInst = PriorityMuxDefault(isIllegalInstVec.zip(io.out.map(_.bits)), 0.U.asTypeOf(new DecodeOutUop))
  ...
  val finalDecodedInst = Wire(Vec(DecodeWidth, new DecodeOutUop))
